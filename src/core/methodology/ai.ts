...
async function fixJsonDataWithAI(data: any, originalData: any) {
  const fixPrompt = `
 ...
 */
async function suggest(data: any, methodology: any): Promise<any> {
 ...
 * @throws {Error} If the OpenAI API is not configured properly
 */
async function reviewData(data: any, methodology: any): Promise<{review_comment: string, review_score: number}> {
  const startTime = Date.now();
...
`;
  ...
  const reviewSchema = z.object({
    review_comment: z.string().describe('Detailed review comment with compliance assessment and suggestions'),
    review_score: z.number().min(0).max(100).describe('Compliance score from 0-100')
  });
  ...
  try {
    ...
    try {
      ...
      if (jsonMatch && jsonMatch[1]) {
        ...
        if (typeof parsed.review_comment === 'string' && 
            typeof parsed.review_score === 'number' &&
            parsed.review_score >= 0 && 
            parsed.review_score <= 100) {
          const endTime = Date.now();
          console.log(`Review completed in ${endTime - startTime}ms (fallback mode)`);
          console.log(`Review score: ${parsed.review_score}/100`);
          return parsed;
        }
      }
      
      throw new Error('Failed to extract valid review structure from AI response');
    } catch (fallbackError) {
