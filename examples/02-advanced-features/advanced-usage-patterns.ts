...
import { Contact, Flow, Process } from '../../src/types';
...
contacts[0].contactDataSet.contactInformation.dataSetInformation['common:name'] = [
  { '@xml:lang': 'en', '#text': 'Dr. Alice Johnson' }
];
contacts[1].contactDataSet.contactInformation.dataSetInformation['common:name'] = [
  { '@xml:lang': 'en', '#text': 'Prof. Bob Wilson' }
];
contacts[2].contactDataSet.contactInformation.dataSetInformation['common:name'] = [
  { '@xml:lang': 'en', '#text': 'Ms. Carol Davis' }
];

console.log(`Created ${contacts.length} contacts`);
contacts.forEach((contact, index) => {
  const name = contact.contactDataSet.contactInformation.dataSetInformation['common:name']?.[0]?.['#text'];
...
combustionProcess.processDataSet.processInformation.dataSetInformation['common:name'] = [
  { '@xml:lang': 'en', '#text': 'Natural gas combustion' }
];

// Add CO2 as an output exchange
const co2FlowUUID = co2Flow.flowDataSet.flowInformation.dataSetInformation['common:UUID'];
combustionProcess.processDataSet.exchanges.exchange = [{
  ...
  referenceToFlowDataSet: {
    ...
    '@version': '1.0.0',
    '@uri': '',
    'common:shortDescription': [{ '@xml:lang': 'en', '#text': 'Carbon dioxide' }]
  },
...
console.log('- Unit Group:', massUnitGroup.unitGroupDataSet.unitGroupInformation.dataSetInformation['common:name']?.[0]?.['#text']);
console.log('- Flow Property:', massFlowProperty.flowPropertyDataSet.flowPropertiesInformation.dataSetInformation['common:name']?.[0]?.['#text']);
console.log('- Flow:', co2Flow.flowDataSet.flowInformation.dataSetInformation['common:name']?.[0]?.['#text']);
console.log('- Process:', combustionProcess.processDataSet.processInformation.dataSetInformation['common:name']?.[0]?.['#text']);
...
console.log('Exported contact JSON size:', exportedContactJSON.length, 'characters');
...
const originalName = contacts[0].contactDataSet.contactInformation.dataSetInformation['common:name']?.[0]?.['#text'];
const importedName = importedContact.contactDataSet.contactInformation.dataSetInformation['common:name']?.[0]?.['#text'];
...
if (!validationResult.success) {
  ...
  console.log('Number of validation errors:', validationResult.error.errors.length);
} else {
...
console.log('Complete flow validation:', completeValidation.success ? 'PASSED' : 'FAILED');
...
const originalContactName = originalContact.contactDataSet.contactInformation.dataSetInformation['common:name']?.[0]?.['#text'];
const clonedContactName = clonedContact.contactDataSet.contactInformation.dataSetInformation['common:name']?.[0]?.['#text'];

console.log('Original contact:', originalContactName);
console.log('Cloned contact:', clonedContactName);
console.log('UUIDs are different:', 
  originalContact.contactDataSet.contactInformation.dataSetInformation['common:UUID'] !== 
  clonedContact.contactDataSet.contactInformation.dataSetInformation['common:UUID']
);
...
multiLangFlow.flowDataSet.flowInformation.dataSetInformation['common:generalComment'] = [
...
multiLangFlow.flowDataSet.flowInformation.dataSetInformation['common:name']?.forEach(name => {
...
console.log(`Created and configured ${manyFlows.length} flows in ${(endTime - startTime).toFixed(2)}ms`);
...
const successfulValidations = validationResults.filter(result => result).length;
console.log(`Validated ${manyFlows.length} flows in ${(validationEndTime - validationStartTime).toFixed(2)}ms`);
console.log(`${successfulValidations}/${manyFlows.length} flows passed validation`);
