...
- Q: 当 submodule 更新包含需要手动调整 SDK 代码的破坏性更改时，流水线应如何处理？ → A: 失败运行并阻止发布，要求手动干预
- Q: 当流水线在短时间内被多次触发时应如何处理新的模式提交？ → A: 取消当前执行并针对最新提交重新启动流水线
- Q: 当自动计算的版本已存在时应如何处理？ → A: 失败发布并要求人工解决版本冲突

### 边缘情况

- 当 submodule 更新包含需要手动调整 SDK 代码的破坏性模式更改时，流水线应失败、阻止后续阶段并创建手动干预任务，确保不会自动发布不完整的 SDK
- 系统如何处理部分失败（例如 Python SDK 验证通过但 TypeScript SDK 失败）？
- 当流水线在短时间内被多次触发时，如果出现新的模式提交，正在运行的流水线应中止并立即重新针对最新提交启动新执行，避免发布过期制品
- 系统如何处理发布到包注册表时的身份验证/凭证失败？
- 当网络问题阻止 submodule 更新或包注册表访问时会发生什么？
- 当自动计算的版本号已在注册表存在时，流水线应终止发布并提示人工介入解决版本冲突，确保语义化版本纪律被遵守
