...
class EntityModelGenerator:
    ...
    def generate_entity_model(self, schema_name: str) -> str:
        ...
        model_name = entity_name.replace(
            "Tidas", ""
        )  # Get the model name without 'Tidas' prefix

        # Generate imports
        lines = [
            '"""',
            f"{entity_name} entity wrapper class for TIDAS SDK. Automatically generated by generate_entity_models.py script. Do not edit this file manually.",
            ...
            f"from ..core.wrappers.{schema_name.replace('tidas_', '')}_wrappers import {model_name}DataSetWrapper",
            "from ..core.validation import ValidationConfig",
            f"from ..types.{schema_name} import Model as {model_name}Model",
            "",
            "",
            f"class {entity_name}(TidasEntity):",
            f'    """Wrapper class for {model_name} entities with pythonic API.',
            "",
            f"    Represents a {model_name.lower()} in LCA.",
            "",
            "    Example:",
        ...
        if schema_name == "tidas_processes":
            ...
            lines.extend(
                [
                    f"        >>> {entity_name.lower().replace('tidas', '')}.{model_name.lower()}_data_set.{model_name.lower()}_information.data_set_information.name.set_text(",
                    f'        ...     "Example {model_name}", "en"',
                    "        ... )",
                ]
            )
        ...
        lines.extend(
            [
                ...
                "    ):",
                f'        """Initialize {model_name} entity.',
                "",
                "        Args:",
                f"            data: {model_name} data as dictionary (TIDAS JSON format)",
                "            validation_config: Validation configuration. If None, uses global default.",
                '        """',
                "        super().__init__(data, validation_config)",
                f"        self._pydantic_model = {model_name}Model",
                "",
        ...
        if schema_name == "tidas_processes":
            ...
            if schema_name == "tidas_unitgroups":
                lines.append('                    "units": {},')
        ...
        lines.extend(
            [
                "",
                f"    @property",
                f"    def {model_name.lower()}_data_set(self) -> {model_name}DataSetWrapper:",
        ...
        if schema_name == "tidas_processes":
            lines.extend(
                [
                    f'            >>> {entity_name.lower().replace("tidas", "")}.{model_name.lower()}_data_set.process_information.data_set_information.uuid = "..."',
                ]
            )
        else:
            lines.extend(
                [
                    f'            >>> {entity_name.lower().replace('tidas', '')}.{model_name.lower()}_data_set.{model_name.lower()}_information.data_set_information.uuid = "..."',
                ]
            )
        ...
        lines.extend(
            [
                '        """',
                f'        return self._get_typed_field("{dataset_name}", {model_name}DataSetWrapper)',
                "",
                f"    def to_pydantic(self) -> {model_name}Model:",
                ...
                "",
                f"    @classmethod",
                f"    def from_pydantic(cls, model: {model_name}Model) -> '{entity_name}':",
    ...
    def generate_all_entity_models(self, output_dir: Path) -> None:
        ...
        # Generate factory methods for direct Pydantic model creation
        factory_file = output_dir.parent / "factories.py"
        if factory_file.exists():
            logger.info(f"Updating factory file: {factory_file}")
            self.update_factory_file(factory_file)
        else:
            logger.warning(f"Factory file not found: {factory_file}")

    def update_factory_file(self, factory_file: Path) -> None:
        """Update factory file with new methods for direct Pydantic model creation.

        Args:
            factory_file: Path to the factories.py file
        """
        with open(factory_file, "r", encoding="utf-8") as f:
            content = f.read()

        # Check if we already have the new methods
        if "def create_process_model(" in content:
            logger.info("Factory file already contains new methods")
            return

        # Find the position to insert new methods (after create_processes_batch)
        insert_pos = content.find("def create_processes_batch(")
        if insert_pos == -1:
            logger.error("Could not find create_processes_batch method")
            return

        # Find the end of the create_processes_batch method
        end_pos = content.find("\n\n", insert_pos)
        if end_pos == -1:
            # Find the next method instead
            end_pos = content.find("\ndef ", insert_pos + 1)

        if end_pos == -1:
            logger.error("Could not find end of create_processes_batch method")
            return

        # Generate new factory methods
        new_methods = self.generate_factory_methods()

        # Insert new methods
        updated_content = content[:end_pos] + "\n\n" + new_methods + content[end_pos:]

        with open(factory_file, "w", encoding="utf-8") as f:
            f.write(updated_content)

        logger.info("  ✓ Updated factory file with new methods")

    def generate_factory_methods(self) -> str:
        """Generate new factory methods for direct Pydantic model creation.

        Returns:
            String containing the new factory methods
        """
        return '''
